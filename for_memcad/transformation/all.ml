...
let transform_decl clang decl =
  ...
  |> log "NoContinueInFor" (* ! must be before ForToWhile ! *)
  ...
  |> log "AssignCond" (* ! must be before LiftConditionals ! *)
    (AssignCond.transform_decl clang)
  |> log "LiftConditionals" (LiftConditionals.transform_decl clang)
  |> log "PostIncrDecr" (PostIncrDecr.transform_decl clang)
  |> log "PreIncrDecr" (* ! must be after PostIncrDecr ! *)
